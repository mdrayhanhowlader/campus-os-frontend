...
const THREADS = [
  { id: '1', name: 'Mr. Verma (Math)',     preview: 'Your assignment has been graded',     time: '11:30 AM', unread: 1 },
  { id: '2', name: 'Class Teacher',        preview: 'Please bring your parent tomorrow',   time: 'Yesterday', unread: 2 },
  { id: '3', name: 'Admin Office',         preview: 'Hall ticket is now available',         time: 'Apr 20', unread: 0 },
];
const MESSAGES: Record<string, { from: string; text: string; mine: boolean; time: string }[]> = {
  '1': [
    { from: 'Mr. Verma', text: 'I have graded your Quadratic Equations assignment. You scored 18/20. Well done!', mine: false, time: '11:30 AM' },
    { from: 'You', text: 'Thank you, sir! I will work on the mistakes.', mine: true, time: '11:45 AM' },
  ],
  '2': [
    { from: 'Class Teacher', text: 'Please ensure your parent or guardian comes to school tomorrow for the parent-teacher meeting at 10 AM.', mine: false, time: 'Yesterday' },
  ],
  '3': [
    { from: 'Admin Office', text: 'Your hall ticket for the mid-term examinations is now available in the Exams section of the portal. Please download and bring it.', mine: false, time: 'Apr 20' },
    { from: 'You', text: 'Thank you! I have downloaded it.', mine: true, time: 'Apr 20' },
  ],
};
...
export default function StudentMessagesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Messages" description="Messages from your teachers and school." breadcrumbs={[{ label: 'Dashboard', href: '/student' }, { label: 'Messages' }]} />
      <Card className="border-0 shadow-sm overflow-hidden">
        <div className="flex h-[520px]">
          <div className="w-64 shrink-0 border-r dark:border-gray-800 flex flex-col">
            <div className="p-3 border-b dark:border-gray-800">
              <div className="relative"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search..." className="pl-8 h-8 text-sm" /></div>
            </div>
            <div className="flex-1 overflow-y-auto divide-y dark:divide-gray-800">
              {THREADS.map((t) => (
                <button key={t.id} onClick={() => setActive(t.id)} className={cn('w-full flex items-start gap-3 p-3 text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors', active === t.id && 'bg-blue-50 dark:bg-blue-950/30')}>
                  <Avatar className="h-8 w-8 shrink-0 mt-0.5"><AvatarFallback className="text-xs bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300">{getInitials(t.name)}</AvatarFallback></Avatar>
          ...
          <div className="flex flex-1 flex-col">
            <div className="flex items-center gap-3 border-b px-4 py-3 dark:border-gray-800">
              <Avatar className="h-8 w-8"><AvatarFallback className="text-xs bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300">{getInitials(thread.name)}</AvatarFallback></Avatar>
